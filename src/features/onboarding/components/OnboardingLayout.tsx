...
type OnboardingLayoutProps = {
  ...
  primary?: { label: string; onClick: () => void } | undefined;
  children: ReactNode;
};
      ...
      <div className="relative z-[2] flex min-h-dvh flex-col px-6 pb-6 lg:min-h-0 lg:justify-center lg:px-14 lg:py-12">
        <div className="mobile-safe-top flex items-center justify-between pt-6 lg:mb-8 lg:pt-0">
          ...
          {onSkip && (
            <button
              type="button"
              onClick={onSkip}
              className="rounded-md px-1 py-1.5 text-sm font-bold text-text-muted transition hover:text-text focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring lg:hidden"
            >
              {skipLabel}
            </button>
          )}
        ...
        {primary && (
          <div className="mobile-safe-bottom flex flex-col items-stretch gap-3 pt-4 lg:mt-10 lg:flex-row lg:items-center lg:gap-4 lg:pt-0">
            <Button size="lg" fullWidth onClick={primary.onClick} className="lg:w-auto lg:px-9">
              {primary.label}
            ...
            {onSkip && (
              <button
                type="button"
                onClick={onSkip}
                className="hidden rounded-md px-2 py-2 text-base font-bold text-text-muted transition hover:text-text focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring lg:inline"
              >
                {skipLabel}
              </button>
            )}
